...
class BusOpLogin extends Component {
    ...
    checkBusop(event) {
        ...
        this.props.onCheckBusop({id: 0, username: this.username.current.value, password: this.password.current.value});
    }
        ...
        return (
            <Grid container spacing={0} direction="column" alignItems="center" justify="center" style={{position: 'absolute', top: '50%', transform: 'translateY(-50%)'}}>
                ...
                <CardContent> 
                    ...
                    <Link to={"/home"}><Button color="primary">Login</Button></Link><br /><br/>
                    </div>
                </CardContent>
                </Card>
                </Grid>      
...
const mapDispatchToState = (dispatch) => {
    return {
        onCheckBusop: (username, password) => dispatch(actions.checkBusop(username, password))
    }
}

export default connect(mapStateToProps, mapDispatchToState)(BusOpLogin);